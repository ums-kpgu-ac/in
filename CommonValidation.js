...
function ValidateAttachments(className, validationgroup) {
    debugger;
    if (className == null || className == "") {
        className = "fuTaskAttachments";
    }

    if (Page_ClientValidate(validationgroup)) {
        debugger;
        var allowedFileTypes = ['pdf', 'doc', 'docx', 'txt', 'xlsx', 'ppt', 'pptx', 'zip', 'jpg', 'png', 'bmp', 'jpeg'];
        var message = "";
        $("." + className).each(function (index) {
            var fileuploadcontrol = this;
            if (fileuploadcontrol.value != "") {
                var Extension = fileuploadcontrol.value.substring(fileuploadcontrol.value.lastIndexOf('.') + 1).toLowerCase();
                if (allowedFileTypes.indexOf(Extension) <= -1) {
                    message += "The file at Attachment No. " + parseInt(index + 1) + " is invalid (Allowed file types: pdf, doc, docx, txt, xlsx, ppt, zip) \n";
                }
            }
        });
...
function IsStudentName(event) { 
    // Backspace, tab, enter, end, home, left, right in number part,
    // We don't support the del key in Opera because del == . == 46.
    //45 -Dash,  39-Coma 46-Dot  39-'
    //var controlKeys = [8, 9, 13,110,32];--remove 110(.) for allowing dot(.)
    ...
    if (!event.which || // Control keys in most browsers. e.g. Firefox tab is 0
        ...
        event.which === 46 ||// Always A through B
        isControlKey) { // Opera assigns values for control keys.
        return;
    } else {
        event.preventDefault();
...
function IsDecimal2DigitPositive(control, event) {
    ...
    if (!event.which || // Control keys in most browsers. e.g. Firefox tab is 0
        ...
        {
            if (control.value.toString().slice(control.value.toString().indexOf('.'), control.value.toString().length).length > 2)
                return false;
        }
        ...
        if (
                (
                    ...
                    &&
                    (control.value.toString().length - control.value.toString().indexOf('.')) > 2
                )
                &&
...
function IsDecimal4DigitPositive(control, event) {
    // Backspace, tab, enter, end, home, left, right in number part, 
    // We don't support the del key in Opera because del == . == 46.
    ...
    if (!event.which || // Control keys in most browsers. e.g. Firefox tab is 0
        ...
        if (control.value.toString().indexOf('.') != -1) {
            if (control.value.toString().slice(control.value.toString().indexOf('.'), control.value.toString().length).length > 4)
                return false;
        }
